...
public class Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		try {
			final File folder = new File("C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\original csv");
			for (final File fileEntry : folder.listFiles()) {
				System.out.println(fileEntry.getName());

				br = new BufferedReader(new FileReader(fileEntry.getAbsolutePath()));

				inputList = br.lines().skip(1).map(mapToItem).filter(newInvite -> newInvite.days == 5)
			...
			//writeRandomResult(minimumIvitations);
			// writeResultByCompany(minimumByCompany);
 			writeResulyByCity(minimumByCities);
