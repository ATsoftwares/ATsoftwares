...
public class oldmain {
	...
	static HashMap<String, HashMap<String, HashMap<Integer, List<hotelinvitation>>>> dictionaryInvitations;
	...
	public static void main(String[] args) throws Exception {
		...
		dictionaryInvitations = new HashMap<String ,HashMap<String, HashMap<Integer, List<hotelinvitation>>>>();
		try {
			...
			for (final File fileEntry : folder.listFiles()) {
				...
				for (hotelinvitation newInvite : inputList) {
					...
					if (dictionaryInvitations.containsKey(newInvite.hotelName)) {
						...
						if (dictionaryInvitations.get(newInvite.hotelName).containsKey(newInvite.checkInFull)) {

							// the hotel exist, it is in the same month,
							// check if it is in the same day in week
							if (dictionaryInvitations.get(newInvite.hotelName).get(newInvite.checkInFull)
									.containsKey(newInvite.daysDiff)) {
								...
								dictionaryInvitations.get(newInvite.hotelName).get(newInvite.checkInFull)
										.get(newInvite.daysDiff).add(newInvite);
			...
			for (HashMap<String, HashMap<Integer, List<hotelinvitation>>> hotels : dictionaryInvitations.values()) {
				for (HashMap<Integer, List<hotelinvitation>> months : hotels.values()) {
					for (List<hotelinvitation> daysInWeek : months.values()) {
						...
						if (isGroupValid(daysInWeek)) {
							...
							List<hotelinvitation> minimumDesc = new ArrayList<>();
							minimumDesc.add(getMinimumOfDesc("A", daysInWeek));
							minimumDesc.add(getMinimumOfDesc("B", daysInWeek));
							minimumDesc.add(getMinimumOfDesc("C", daysInWeek));
							minimumDesc.add(getMinimumOfDesc("D", daysInWeek));
							minimumDesc.add(getMinimumOfDesc("E", daysInWeek));
							dictionaryInvitations.get(daysInWeek.get(0).hotelName).get(daysInWeek.get(0).checkInFull).put((int) daysInWeek.get(0).daysDiff, minimumDesc);

							for (hotelinvitation invite : minimumDesc) {
			...
			//printMaxSpliteToHotel(minimumByHotels);
			//writeAllSplitesByDays(minimumIvitations);
			//writeAllSplitesByDiffDays(minimumIvitations);
			//printMinimumWithAllFields(minimumIvitations);
			
//			writeMinimumAgainsE(minimumIvitations);
			// writeAllData();

					//writeResultByHotelArff(minimumByHotels);
					writeResultByCompanyArff(minimumByCompany);
//					writeRandomResultWithoutE(minimumByHotels, dictionaryInvitations);
...
//					dictionaryInvitations);
	...
	static void addDayDiff(
		...
		list.add(newInvite);
		dictionaryInvitations.get(newInvite.hotelName).get(newInvite.checkInFull).get(newInvite.daysDiff)
				.put((long) newInvite.days, list);
	}

	static void addDayInMonth(
			HashMap<String, HashMap<String, HashMap<Integer, List<hotelinvitation>>>> dictionaryInvitations,
			hotelinvitation newInvite) {
		List<hotelinvitation> list = new ArrayList<hotelinvitation>();
		list.add(newInvite);
		dictionaryInvitations.get(newInvite.hotelName).get(newInvite.checkInFull).put((int) newInvite.daysDiff, list);
	...
	static void addMonth(
		...
		dictionaryInvitations.get(newInvite.hotelName).put(newInvite.checkInFull, a);
		addDayInMonth(dictionaryInvitations, newInvite);
	}
	...
	static void addHotel(
			HashMap<String, HashMap<String, HashMap<Integer, List<hotelinvitation>>>> dictionaryInvitations,
			hotelinvitation newInvite) {
		HashMap<String, HashMap<Integer, List<hotelinvitation>>> a = new HashMap<String, HashMap<Integer, List<hotelinvitation>>>();
	...
	static void writeResulyByCity(HashMap<String, List<hotelinvitation>> minimumByHotels, List<hotelinvitation> min) {
		List<hotelinvitation> wekaLearn = IntStream.range(0, min.size()).filter(i -> i % 4 != 0)
				.mapToObj(i -> min.get(i)).collect(Collectors.toList());
		List<hotelinvitation> wekaTest = IntStream.range(0, min.size()).filter(i -> i % 4 == 0)
				.mapToObj(i -> min.get(i)).collect(Collectors.toList());
...
//
//			List<hotelinvitation> wekaLearn = IntStream.range(0, currHotel.size()).filter(i -> i % 4 != 0)
//					.mapToObj(i -> currHotel.get(i)).collect(Collectors.toList());
//			List<hotelinvitation> wekaTest = IntStream.range(0, currHotel.size()).filter(i -> i % 4 == 0)
//					.mapToObj(i -> currHotel.get(i)).collect(Collectors.toList());
//			String folderName = "˛˛wekaPredictions2";
//			writeCSVFileToHotel(wekaLearn, folderName, true, "wekaLearnCSV");
//			writeCSVFileToHotel(wekaTest, folderName, true, "wekaTestCSV");
...
//				sb.append(invite.checkInFull + ",");
	...
	static void writeAllSplitesByDays(List<hotelinvitation> minimum){
		hotelinvitation tt = new hotelinvitation();
		try {
		int [][] descriptionByDayInWeek = new int [ 8 ] [ 5 ] ; 
		for (hotelinvitation invite :minimum){
			tt = invite;
			descriptionByDayInWeek[invite.DayInWeek][getIndexOfDescription(getDescriptoin(invite.descriptionPartOne))]++;
		}
		
		String url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\descriptionByDayInWEek.csv";
		PrintWriter pw;
...
//			sb.append("hotelName,");
		...
		for (int i = 0; i < descriptionByDayInWeek.length; i++) {
			sb.append(i +",");
			
			for (int j = 0; j < descriptionByDayInWeek[i].length; j++) {
				if (j > 0){
					sb.append(",");
				}
				sb.append(descriptionByDayInWeek[i][j] + ",");
				sb.append('\n');
			}
		}
		pw.write(sb.toString());
	...
	static void writeAllSplitesByDiffDays(List<hotelinvitation> minimum){
		hotelinvitation tt = new hotelinvitation();
		try {
		int [][] descriptionByDayInWeek = new int [ 51 ] [ 5 ] ; 
		for (hotelinvitation invite :minimum){
			tt = invite;
			long daysBetween;
			if (invite.runDate != null && invite.cheakIn != null) {
			...
			long diffDays = daysBetween / (24 * 60 * 60 * 1000);
			descriptionByDayInWeek[getIndexByDaysDiff(diffDays)][getIndexOfDescription(getDescriptoin(invite.descriptionPartOne))]++;
		}
		
		String url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\descriptionByDaysDiff.csv";
		PrintWriter pw;
...
//			sb.append("hotelName,");
	...
	static int getIndexByDaysDiff(long daysDiff){
		if (daysDiff ==1){
			return 1;
		}
		if (daysDiff == 2){			
		...
		if(daysDiff > 30 && daysDiff <= 40){
			return 40;
		}
		return 50;
	}
	
	static int getIndexOfDescription(String desc){
		if (desc.equals("A")){
			return 0;
		}
		if (desc.equals("B")){
	...
	static void writeRandomResultWithoutE(HashMap<String,List<hotelinvitation>> minimum,
			HashMap<String, HashMap<String, HashMap<Integer, List<hotelinvitation>>>> dictionaryInvitations) {
		String url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\randomAll2.csv";
		PrintWriter pw;
		try {
			pw = new PrintWriter(new File(url));
			StringBuilder sb = new StringBuilder();
			...
			sb.append("diffPrice from original,");
			sb.append("diffPricePercent from original,");
			sb.append('\n');
			pw.write(sb.toString());
			int counter = 0;
			int counterWithoutE = 0;
			double sumPriceDiff = 0;
			double sumPriceDiffFromOrigin = 0;
			String alphabet = "ABCD";
			Random rand = new Random();
			int evenCounter = 0;
			for (List<hotelinvitation> hotelinvitations : minimum.values() )
				if (isAmountOfELegal(hotelinvitations)){					
					for (hotelinvitation invite : hotelinvitations) {
						if (!getDescriptoin(invite.descriptionPartOne).equals("E")){
							counterWithoutE++;
							sb = new StringBuilder();
							evenCounter++;
							...
							try{
								List<hotelinvitation> invitesFromSameType = dictionaryInvitations.get( invite.hotelName)
										.get(invite.checkInFull).get(invite.daysDiff);
								for (hotelinvitation currInvinte : invitesFromSameType) {
									if (getDescriptoin(currInvinte.descriptionPartOne).indexOf(randDesc) != -1) {
										...
										originalPrice = currInvinte.originalPrice;
									}
								}
							}
							catch(Exception e){						
							...
							sb.append(  randPrice - originalPrice  + ",");
							sb.append(randPrice / originalPrice   + ",");
							sumPriceDiff += originalPrice / randPrice ;
							sb.append('\n');
							sumPriceDiffFromOrigin += randPrice / price;
			...
			sb.append(minimum.size() + ",");
			sb.append((double) counter / (double) counterWithoutE + ",");
			sb.append(sumPriceDiff / (double) counterWithoutE+ ",");
//			sb.append((double) counter / (double) counterWithoutE + ",");
			sb.append(sumPriceDiffFromOrigin / (double) counterWithoutE+ ",");
			pw.write(sb.toString());
			System.out.println("done!!!!");
			pw.close();
		} catch (FileNotFoundException e) {
	...
	static boolean isAmountOfELegal(List<hotelinvitation> invations){
		int counter = 0;
		for (hotelinvitation invite : invations){
			if (getDescriptoin(invite.descriptionPartOne).equals("E")){
				counter++;
			}
		}
		//String s = new String (counter / invations.size());
		double precant =(counter * 1.0) / invations.size();
		return precant < 30;
	}
	
	static void printMinimumWithAllFields (List<hotelinvitation> invitations){
		for (hotelinvitation invite : invitations){
			String a = invite.getAllFields();
			String b = invite.getAllFieldsValues(invite);
			System.out.println(b);
		}
//		String url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\Minimum with all fields.csv";
//		PrintWriter pw;
//		try {
//			pw = new PrintWriter(new File(url));
//			StringBuilder sb = new StringBuilder();
...
//			for (hotelinvitation invite : invitations ){
//				sb.append(invite.hotelName + ",");
//				sb.append(invite.availibleRooms + ",");
//				sb.append(invite.avalibleRoomsPartOne + ",");
//				sb.append(invite.avalibleRoomsPartTwo + ",");
//				sb.append(invite.boardType + ",");
//				sb.append(invite.boardTypePartOne + ",");
//				sb.append(invite.boardTypePartTwo + ",");
//				sb.append(invite.checkInFull + ",");
//				sb.append(invite.hotelName + ",");
//				sb.append(invite.hotelName + ",");
//				sb.append(invite.hotelName + ",");
...
//		}
	...
	static void writeRandomResult(List<hotelinvitation> minimum,
		...
		try {
			...
			sb.append(minimum.size() + ",");
			sb.append(minimum.size() + ",");
			sb.append((double) counter / (double) minimum.size() + ",");
			sb.append(sumPriceDiff / minimum.size());
			pw.write(sb.toString());
			System.out.println("done!!!!");
			pw.close();
		} catch (FileNotFoundException e) {
	...
	static void printMaxSpliteToHotel (HashMap<String, List<hotelinvitation>> minimumHotels){
		long daysBetween;
		String url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\maxPerHotel.csv";
		int [] counterRange = new int [6];
		PrintWriter pw;
		try {
			pw = new PrintWriter(new File(url));
			StringBuilder sb = new StringBuilder();
		...
		for(List<hotelinvitation> hotel : minimumHotels.values()){
			hotelinvitation maxDiff = getMaxDiffPrice(hotel);
			if (!getDescriptoin(maxDiff.descriptionPartOne).equals("E")){				
				sb = new StringBuilder();
				sb.append(maxDiff.hotelName + ",");
				sb.append(maxDiff.month +",");
				sb.append(maxDiff.DayInWeek+",");
				sb.append(maxDiff.dayInMonth+",");
				sb.append(maxDiff.checkInFull +", ");
				if (maxDiff.runDate != null && maxDiff.cheakIn != null) {
					
					daysBetween = maxDiff.cheakIn.getTime() - maxDiff.runDate.getTime();
				} else {
					daysBetween = 0;
				}
				long diffDays = daysBetween / (24 * 60 * 60 * 1000);
				sb.append(diffDays + ",");
				sb.append(getDescriptoin(maxDiff.descriptionPartOne)+ ",");
				double diffPrice =maxDiff.totalPriceForCombine != -1 ? (  maxDiff.originalPrice /(maxDiff.totalPriceForCombine * 1.0)) : 1;
				counterRange[getIndexByRangeMaxSplite((diffPrice -1)*100)]++;
				sb.append(diffPrice + ",");
				sb.append((diffPrice -1)*100+ ",");
				sb.append('\n');
				pw.write(sb.toString());
			}
		}
			pw.close();
			url = "C:\\Users\\Ofra\\Desktop\\ËÏ Â‚Ï\\hotels research\\randomResult\\maxPerHotelRange.csv";
			pw = new PrintWriter(new File(url));
			sb = new StringBuilder();
			sb.append("range,");
			sb.append("amount,");
			...
			for (int i = 0; i < counterRange.length; i++) {
				sb.append(i +",");
				sb.append(counterRange[i]);
				sb.append('\n');
			}
			pw.write(sb.toString());
			pw.close();
	...
	static int getIndexByRangeMaxSplite (double maxSplite){
		if (maxSplite < 0.005){
			return 0;
		}
		else if (maxSplite <= 1){
		...
		else if (maxSplite <=10){			
			return 3;
		}
		else if (maxSplite <=25){			
			return 4;
		}
		else{			
			return 5;
	...
	static hotelinvitation getMaxDiffPrice(List<hotelinvitation> hotel) {
		double maxPricePrecent = 0;
		hotelinvitation maxdiffInvite = new hotelinvitation();
		for (hotelinvitation invite : hotel) {
			Double currDiffPrice = invite.totalPriceForCombine != -1
					? (invite.originalPrice / (invite.totalPriceForCombine * 1.0)) : 1;
			if (maxPricePrecent <= currDiffPrice) {
				maxPricePrecent = currDiffPrice;
				maxdiffInvite = invite;
			}
		}
		return maxdiffInvite;
	}
	
	static hotelinvitation getMinimumOfDesc(String description, List<hotelinvitation> invitations) throws Exception {
		double minimum = 999999;
		hotelinvitation minimumInvite = new hotelinvitation();
		for (hotelinvitation hotelinvitation : invitations) {
			
			if(getDescriptoin(hotelinvitation.descriptionPartOne).equals(description)){				
				if (hotelinvitation.totalPriceForCombine != -1 ) {
					if (hotelinvitation.totalPriceForCombine < minimum ){
						minimum = hotelinvitation.totalPriceForCombine;
						minimumInvite = hotelinvitation;
					}
				}
				else{
					if (hotelinvitation.originalPrice < minimum ){
						minimum = hotelinvitation.totalPriceForCombine;
						minimumInvite = hotelinvitation;
					}
				}
		...
		return minimumInvite;
	}
	
	static void writeMinimumAgainsE (List <hotelinvitation> invitations){
		PrintWriter pw = null;
		int counter = 0;
		double sumDifference = 0;
			String url = "";
				url = "C://Users/Ofra/desktop/ËÏ Â‚Ï/hotels research/wekaPredictions2";
				url = MessageFormat.format("C://Users/Ofra/desktop/ËÏ Â‚Ï/hotels research/{0}/{1}.csv", "ÓÒ˜Â˙ ÓÈÒÂÈÈÌ",
						"againstE");
			...
			StringBuilder sb = new StringBuilder();
			sb.append("hotel name, month, day in month, day in week, checll in full, diff days, description,company ,price for combine,original price, diff price, \n");
			pw.write(sb.toString());
			for (hotelinvitation invite : invitations) {
//				if (!getDescriptoin(invite.descriptionPartOne).equals("E"))
//				{	
					...
					double price = invite.totalPriceForCombine == -1 ? invite.originalPrice : invite.totalPriceForCombine;
					sb.append(price + ",");
					sb.append(invite.originalPrice + ",");
					if (invite.totalPriceForCombine != -1){					
						sb.append(((invite.originalPrice / invite.totalPriceForCombine)-1)*100 +",");
						sumDifference +=invite.originalPrice / invite.totalPriceForCombine  ;
					}
					else {
						sb.append("0,");
						sumDifference += 1;
...
//			}
			sb.append("average diff : " + sumDifference / counter);
			pw.write(sb.toString());
			System.out.println("done!!!!");
			}
		 catch (FileNotFoundException e) {
				System.out.println("file cant be open" + e.toString());
			}
		...
		try (BufferedReader br = new BufferedReader(new FileReader(pathToRead))) {
			...
			for (CustomInvite customInvite : inputList) {
				...
				for (hotelinvitation invite : dictionaryInvitations
						.get(("\"" + customInvite.hotelName + "\"").replaceAll("'", "")).get(customInvite.fullCheckIn.replaceAll("/", "-"))
						.get(customInvite.diffDays)) {
	...
	public static Function<String, CustomInvite> mapCustomItem = (line) -> {
		...
		try{
			...
			newInvite.fullCheckIn = invite[11];
			return newInvite;
		}
		catch(Exception e){
	...
	private static Function<String, hotelinvitation> mapToItem = (line) -> {
		...
		Random rand = new Random();
		Integer nRand = rand.nextInt(999-1) + 1;
		Double dRand = nRand / 1000.0;
		...
		newInvite.daysDiff = (int) daysBetween / (24 * 60 * 60 * 1000);
		Calendar c = Calendar.getInstance();
		newInvite.month = newInvite.cheakIn.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getMonthValue();
		...
		newInvite.checkInFull = newInvite.dayInMonth + "-" + newInvite.month + "-"
				+ newInvite.cheakIn.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear();
		if (newInvite.totalPriceForCombine == -1){
			newInvite.originalPrice += dRand;
		}
		else{
			newInvite.totalPriceForCombine += dRand;
		}
		return newInvite;
	};
